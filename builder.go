...
func (b *Builder) Build(prototypeName string, instanceName ...string) *Instance {
	...
	if err != nil {
		panic(fmt.Sprintf("could not build instance of %s %+v: json error: %s", prototypeName, contents, err.Error()))
	}
...
func (b *Builder) Find(table, query string, instanceName ...string) []*Instance {
	...
	if err != nil {
		panic(fmt.Sprintf("could not build query: json error: %s: %+v", err.Error(), queryMap))
	}
	...
	if err != nil {
		panic(fmt.Sprintf("could not unmarshal query result %+v: %s", contents, err.Error()))
	}
	instances := make([]*Instance, 0)
	name := table
	if len(instanceName) > 0 {
